...
public:
    int maximumScore(vector<int>& nums, vector<int>& multipliers)
    {
        ...
        return go(0, 0, nums, multipliers);
    }
    int go(int l, int i, vector<int>& nums, vector<int>& multipliers)
    {
        if (i == m)
            return 0;
        if (dp[l][i] != INT_MIN)
            return dp[l][i];
        int pickHead = go(l + 1, i + 1, nums, multipliers) + multipliers[i] * nums[l];
        int pickTail = go(l, i + 1, nums, multipliers) + multipliers[i] * nums[n - 1 - (i - l)];
        dp[l][i] = max(pickHead, pickTail);
        return dp[l][i];
    }
};
int main(void)

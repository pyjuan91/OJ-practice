...
class Solution {
	public:
		...
		vector<string>
		  findNeighbors(string &word, unordered_set<string> &wordList) {
		...
		void backtrack(string &source, string &destination) {
			...
			if (source == destination) {
				shortestPaths.push_back(vector<string>(
				  currPath.rbegin(), currPath.rend()));
			}
		...
		void bfs(string &beginWord, string &endWord, unordered_set<string> &wordList) {
			...
			while (!q.empty()) {
				...
				for (int i = q.size() - 1; i >= 0; i--) {
					...
					vector<string> neighbors
					  = findNeighbors(currWord, wordList);
		...
		vector<vector<string>>
		  findLadders(string beginWord, string endWord, vector<string> &wordList) {
			...
			unordered_set<string> copiedWordList(

...
void butterfly(std::vector<mint>& a)
{
    ...
    static fft_info<mint> const info;
        ...
        if (len == 1)
        {
            ...
            for (int s = 0; s < (1 << (len - 1)); s++)
            {
                ...
                for (int i = 0; i < p; i++)
                {
                    ...
                    a[i + offset] = l + r;
                    a[i + offset + p]
                        = (unsigned long long) ((unsigned int) (l.val() - r.val()) + mint::mod()) * irot.val();
                    ;
                }
                if (s + 1 != (1 << (len - 1)))
...
template <class mint, internal::is_static_modint_t<mint>* = nullptr>
std::vector<mint> convolution_naive(std::vector<mint> const& a, std::vector<mint> const& b)
{
...
template <class mint, internal::is_static_modint_t<mint>* = nullptr>
std::vector<mint> convolution(std::vector<mint> const& a, std::vector<mint> const& b)
{
    int n = int(a.size()), m = int(b.size());
    if (!n || !m)
        return {};
...
std::vector<long long> convolution_ll(std::vector<long long> const& a, std::vector<long long> const& b)
{
    int n = int(a.size()), m = int(b.size());
    if (!n || !m)
        return {};
